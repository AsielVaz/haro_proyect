...
.haro-search-source[data-haro-search-ready="1"] {
...
.haro-auto-search-select.is-open {
    z-index: 100000;
}
...
.haro-auto-search-select.is-open .haro-auto-search-dropdown {
    display: block;
}
...
<style id="haro-gallery-bulk-assign">
/* ==========================================================
   MODO ASIGNACIÓN MASIVA
   1 auto global + imágenes palomeables.
   El envío conserva: accion=asignar, id=imagen, auto=id_auto.
========================================================== */
.auto-global-card {
    background: var(--haro-paper, #fff);
    border: 1px solid var(--haro-line-gold, rgba(201,162,74,.20));
    border-radius: 22px;
    box-shadow: var(--haro-shadow, 0 18px 48px rgba(15,15,18,.09));
    padding: 18px;
    margin-bottom: 22px;
    display: grid;
...
.auto-global-card__select:focus {
    border-color: var(--haro-red, #b0141b);
    box-shadow: 0 0 0 4px rgba(176,20,27,.10);
}

.auto-global-card__counter {
    min-width: 160px;
    min-height: 48px;
    border-radius: 16px;
    background: rgba(176,20,27,.08);
    border: 1px solid rgba(176,20,27,.15);
    color: var(--haro-red, #b0141b);
    display: flex;
    align-items: center;
    justify-content: center;
...
.img-card.is-selected {
    border-color: rgba(176,20,27,.55) !important;
    box-shadow: 0 24px 58px rgba(176,20,27,.16) !important;
    transform: translateY(-5px);
}

.img-card.is-selected .img-card__thumb::before {
    content: "";
    position: absolute;
    inset: 0;
    background: rgba(176,20,27,.16);
    z-index: 1;
    pointer-events: none;
}
...
.img-card__check-box {
    ...
    transition: background var(--haro-transition, 220ms ease), color var(--haro-transition, 220ms ease), transform var(--haro-transition, 220ms ease);
}

.img-card__check input:checked + .img-card__check-box {
    background: linear-gradient(135deg,var(--haro-red,#b0141b),#7d1118);
    color: #fff;
...
.btn-select-card {
    flex: 1;
    background: rgba(176,20,27,.10);
    border: 1px solid rgba(176,20,27,.18);
    color: var(--haro-red, #b0141b);
    border-radius: 14px;
    cursor: pointer;
    font-family: 'Bebas Neue', sans-serif;
...
.img-card.is-selected .btn-select-card {
    background: linear-gradient(135deg,var(--haro-red,#b0141b),#7d1118);
    color: #fff;
...
@media(max-width:768px){
    .auto-global-card{grid-template-columns:1fr;}
    .auto-global-card__counter{width:100%;}
}
</style>
...
<body class="layout-boxed enable-secondaryNav">
                ...
                <div class="middle-content container-xxl p-0">
                    <div class="gallery-wrapper">
                        ...
                        <div class="stats-bar">
                            ...
                            <div class="stat-pill">
                                <div class="dot" style="background:#b0141b"></div>
                                <strong id="selectedCountStat">0</strong> seleccionadas
                            </div>
                        </div>
                        ...
                        <!-- Selector global de auto -->
                        <div class="auto-global-card">
                            <div>
                                <p class="auto-global-card__label">Vehículo destino</p>
                                <select id="autoGlobal" class="auto-global-card__select" data-haro-search="1" data-search-placeholder="Buscar vehículo..." onchange="seleccionarAutoGlobal(this.value)">
                                    <option value="0">Selecciona primero un vehículo…</option>
                                ...
                                <div class="auto-global-card__help">Después palomea las imágenes que pertenecen a este auto y presiona guardar.</div>
                            </div>
                            <div id="selectedCountBox" class="auto-global-card__counter">0 imágenes</div>
                        </div>
                        ...
                        <div class="gallery-grid" id="galleryGrid">
                            ...
                            <div class="img-card" data-id="<?php echo $imagen->id; ?>" data-selected="0">
                                ...
                                <div class="img-card__thumb">
                                    ...
                                    <label class="img-card__check" onclick="event.stopPropagation();">
                                        <input type="checkbox"
                                               class="img-check"
                                               data-imagen-id="<?php echo $imagen->id; ?>"
                                               onchange="sincronizarSeleccion(this)">
                                ...
                                <div class="img-card__body">
                                    <div>
                                        <p class="img-card__label">Imagen disponible</p>
                                        <div class="img-card__hint">Palomea esta imagen para asignarla al vehículo seleccionado.</div>
                                    </div>

                                    <div class="img-card__actions">
                                        <button class="btn-select-card"
                                                type="button"
                                                onclick="event.stopPropagation(); toggleCardSeleccion(<?php echo $imagen->id; ?>, this)">
                                            Palomear
                                        </button>
                                        <button class="btn-delete"
                                                type="button"
                                                onclick="event.stopPropagation(); funcionEliminar(<?php echo $imagen->id; ?>, this)">
    ...
    <script>
    /* ── Flujo nuevo: 1 auto global + varias imágenes palomeadas ── */
    var autoSeleccionado = '0';
    var arreglo = [];
    var contador = 0;

    /* ── Select buscable nativo para la lista de autos ──
       No usa NiceSelect2/Select2. Mantiene vivo el <select> original,
       cambia su value y dispara el evento change para conservar el flujo. */
    ...
    function cerrarSelectsBuscables(excepto) {
        document.querySelectorAll('.haro-auto-search-select.is-open').forEach(function (wrap) {
            if (wrap !== excepto) {
                wrap.classList.remove('is-open');
                var trigger = wrap.querySelector('.haro-auto-search-select__button');
                if (trigger) trigger.setAttribute('aria-expanded', 'false');
            }
        });
    }

    function inicializarSelectBuscable(select) {
        if (!select || select.getAttribute('data-haro-search-built') === '1') return;

        select.setAttribute('data-haro-search-built', '1');
        ...
        text.className = 'haro-auto-search-select__text';
        ...
        dropdown.className = 'haro-auto-search-dropdown';

        var searchWrap = document.createElement('div');
        searchWrap.className = 'haro-auto-search-dropdown__search-wrap';

        var search = document.createElement('input');
        search.type = 'text';
        search.className = 'haro-auto-search-dropdown__search';
        search.placeholder = select.getAttribute('data-search-placeholder') || 'Buscar...';
        search.autocomplete = 'off';

        var optionsBox = document.createElement('div');
        optionsBox.className = 'haro-auto-search-dropdown__options';
        ...
        select.classList.add('haro-search-source');
        select.setAttribute('data-haro-search-ready', '1');

        function textoSeleccionado() {
            var option = select.options[select.selectedIndex];
            return option ? String(option.textContent || '').trim() : 'Selecciona una opción';
        }

        function actualizarTexto() {
            text.textContent = textoSeleccionado();
            text.title = textoSeleccionado();
        }

        function elegirOpcion(value) {
            select.value = value;
            actualizarTexto();
            cerrarDropdown();
        ...
        function pintarOpciones(filtro) {
            ...
            Array.prototype.forEach.call(select.options, function (option) {
                ...
                var coincide = !q || normalizarBusqueda(label).indexOf(q) !== -1;
                ...
                if (String(select.value) === String(value)) item.classList.add('is-selected');
                if (option.disabled) item.disabled = true;

                item.addEventListener('click', function () {
                    if (!option.disabled) elegirOpcion(value);
                });
        ...
        function abrirDropdown() {
            cerrarSelectsBuscables(wrapper);
        ...
        button.addEventListener('click', function (e) {
            e.preventDefault();
            wrapper.classList.contains('is-open') ? cerrarDropdown() : abrirDropdown();
        });

        search.addEventListener('input', function () {
            pintarOpciones(search.value);
        });
        ...
        search.addEventListener('keydown', function (e) {
            ...
            if (e.key === 'Enter') {
                e.preventDefault();
                var first = optionsBox.querySelector('.haro-auto-search-dropdown__option:not([disabled])');
                if (first) elegirOpcion(first.getAttribute('data-value'));
            }
        });
    ...
    function inicializarSelectsBuscables() {
        document.querySelectorAll('select[data-haro-search="1"]').forEach(function (select) {
            inicializarSelectBuscable(select);
        });
    }

    document.addEventListener('click', function () {
        cerrarSelectsBuscables(null);
    });

    function seleccionarAutoGlobal(valor) {
        autoSeleccionado = valor || '0';
        reconstruirArreglo();
        actualizarResumenSeleccion();
    }

    function autoValidoSeleccionado() {
        return autoSeleccionado && autoSeleccionado !== '0';
    }

    function filtrarTarjetas(q) {
        var filtro = String(q || '').trim();
        document.querySelectorAll('.img-card').forEach(function (card) {
            card.style.display = (!filtro || String(card.dataset.id).indexOf(filtro) !== -1) ? '' : 'none';
        });
    }

    function setCardSeleccionada(card, seleccionada) {
        if (!card) return;
        card.dataset.selected = seleccionada ? '1' : '0';
        card.classList.toggle('is-selected', !!seleccionada);

        var checkbox = card.querySelector('.img-check');
        if (checkbox) checkbox.checked = !!seleccionada;

        var btn = card.querySelector('.btn-select-card');
        if (btn) btn.textContent = seleccionada ? 'Quitar' : 'Palomear';
    }

    function sincronizarSeleccion(checkbox) {
        if (!checkbox) return;

        if (checkbox.checked && !autoValidoSeleccionado()) {
            checkbox.checked = false;
            Swal.fire({
                icon: 'warning',
                title: 'Selecciona un vehículo',
                text: 'Primero elige el auto al que se asignarán las imágenes.',
                confirmButtonColor: '#2563eb'
            });
            return;
        }

        var card = checkbox.closest('.img-card');
        setCardSeleccionada(card, checkbox.checked);
        reconstruirArreglo();
        actualizarResumenSeleccion();
    }

    function toggleCardSeleccion(imagenId, elemento) {
        if (!autoValidoSeleccionado()) {
            Swal.fire({
                icon: 'warning',
                title: 'Selecciona un vehículo',
                text: 'Primero elige el auto al que se asignarán las imágenes.',
        ...
        var card = elemento && elemento.closest ? elemento.closest('.img-card') : document.querySelector('.img-card[data-id="' + imagenId + '"]');
        if (!card) return;
        ...
        sincronizarSeleccion(checkbox);
    }

    function reconstruirArreglo() {
        arreglo = [];
        if (!autoValidoSeleccionado()) return;

        document.querySelectorAll('.img-check:checked').forEach(function (checkbox) {
            arreglo.push({
                auto: autoSeleccionado,
                imagen: String(checkbox.getAttribute('data-imagen-id'))
            });
        });
    }

    function actualizarResumenSeleccion() {
        var total = document.querySelectorAll('.img-check:checked').length;
        var texto = total + (total === 1 ? ' imagen' : ' imágenes');

        var caja = document.getElementById('selectedCountBox');
        if (caja) caja.textContent = texto;

        var stat = document.getElementById('selectedCountStat');
        if (stat) stat.textContent = total;
    }

    function limpiarSeleccionDeImagen(id) {
        arreglo = arreglo.filter(function (p) { return String(p.imagen) !== String(id); });
        actualizarResumenSeleccion();
    }

    /* ── Guardar todos: se envía igual que antes, imagen por imagen ── */
    function enviarImagen() {
        if (!autoValidoSeleccionado()) {
            Swal.fire({
                icon: 'warning',
                title: 'Selecciona un vehículo',
                text: 'Elige el auto destino antes de guardar.',
                confirmButtonColor: '#2563eb'
            });
            return;
        }
        ...
        if (!arreglo.length) {
            Swal.fire({
                ...
                text: 'Palomea al menos una imagen para guardarla.',
                confirmButtonColor: '#2563eb'
            });
            return;
        }
        ...
        Swal.fire({
            ...
            text: 'Se asignarán ' + arreglo.length + ' imagen(es) al vehículo seleccionado.',
            icon: 'question',
    ...
    function bloquearGuardado(bloquear) {
        var btn = document.querySelector('.fab-save');
        if (!btn) return;
        btn.disabled = !!bloquear;
        btn.style.opacity = bloquear ? '.75' : '1';
        btn.style.pointerEvents = bloquear ? 'none' : '';
        btn.innerHTML = bloquear
            ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 12a9 9 0 1 1-6.2-8.56"/></svg> Guardando...'
            : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> Guardar imágenes';
    }
    ...
    function procesarSiguiente() {
        if (contador < arreglo.length) {
            (async function () {
                ...
                /* IMPORTANTE: estos campos quedan tal cual el flujo anterior */
                ...
                try {
                    ...
                    Swal.fire({
                        ...
                        text: 'No se pudieron enviar todas las imágenes. Intenta de nuevo.',
                        confirmButtonColor: '#2563eb'
                    });
                }
            })();
            ...
            Swal.fire({
                icon: 'success',
                title: '¡Guardado!',
                text: 'Las imágenes seleccionadas fueron asignadas correctamente.',
                confirmButtonColor: '#2563eb',
                timer: 2000,
    ...
    function funcionEliminar(id, btn) {
        Swal.fire({
            ...
            try {
                ...
                if (data == 1) {
                    limpiarSeleccionDeImagen(id);
                    card.style.transition = 'opacity .32s, transform .32s';
                    card.style.opacity    = '0';
                    card.style.transform  = 'scale(0.88)';
                    setTimeout(function () { card.remove(); actualizarResumenSeleccion(); }, 340);
                } else {
