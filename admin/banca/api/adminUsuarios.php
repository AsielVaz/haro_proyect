...
class administradorUsuarios extends conectorB
{
  ...
  function dameUsuariosBanca()
  {
    $arregloUsuarios = array();
    $sql = 'SELECT id, nombre, apellido_paterno, apellido_materno, email, contrasena, telefono, calle, ciudad, pais, direccion, tipo_usuario, imagen, permiso_banca
    FROM usuario where permiso_banca = "Banca";';
    $result = $this->ejecutar($sql);
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        $usuario = new Usuario();
  ...
  function actualizarUsuario($id, $nombre, $apellidoPaterno, $apellidoMaterno, $email, $contrasena, $telefono, $calle, $ciudad, $pais, $direccion)
  {
    ...
    $sql = 'UPDATE usuario SET nombre = "' . $nombre . '", apellido_paterno = "' . $apellidoPaterno . '", apellido_materno = "' . $apellidoMaterno . '", email = "' . $email . '", contrasena = "' . sha1($contrasena) . '", telefono = "' . $telefono . '", calle = "' . $calle . '", ciudad = "' . $ciudad . '", pais = "' . $pais . '", direccion = "' . $direccion . '" WHERE id = ' . $id . ';';
    $this->ejecutar($sql);
  }
